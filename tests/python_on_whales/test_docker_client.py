...
def test_login_logout(registry_without_login):
    busybox_image = docker.pull("busybox")
    busybox_image.tag(f"{registry_without_login}/my_busybox")
    with pytest.raises(DockerException):
        docker.push(f"{registry_without_login}/my_busybox")
    docker.login(registry_without_login, username="my_user", password="my_password")
    assert (
        registry_without_login in (Path.home() / ".docker" / "config.json").read_text()
    )
    docker.push(f"{registry_without_login}/my_busybox")
    docker.push([f"{registry_without_login}/my_busybox" for _ in range(2)])
    docker.pull(f"{registry_without_login}/my_busybox")
    docker.logout(registry_without_login)
    assert (
        registry_without_login
