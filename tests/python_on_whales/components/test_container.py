...
def test_create_with_systemd_mode(
...
def test_create_missing_pull_existent(
    ctr_client: DockerClient, tmp_path: Path, registry: str
):
    base_image_name = "alpine:latest"
    test_image_name = f"{registry}/{base_image_name}"

    base_image = ctr_client.image.pull(base_image_name)
    base_image.tag(test_image_name)
    ctr_client.push(test_image_name)
    ...
    ctr_client.container.create(test_image_name, pull="missing")
    assert ctr_client.image.inspect(test_image_name).id == local_id
...
def test_run_missing_pull_existent(
    ...
    assert remote_id != local_id
    ctr_client.container.run(test_image_name, pull="missing")
    assert ctr_client.image.inspect(test_image_name).id == local_id
...
def test_create_always_pull_existent(
    ...
    assert remote_id != local_id
    ctr_client.container.create(test_image_name, pull="always")
    assert ctr_client.image.inspect(test_image_name).id == remote_id
...
def test_run_always_pull_existent(
    ...
    assert remote_id != local_id
    ctr_client.container.run(test_image_name, pull="always")
    assert ctr_client.image.inspect(test_image_name).id == remote_id
